...
#                 q = retrieval_contriever.src.normalize_text.normalize(q)
...
#                 # Ensure the model output is moved to the correct device
#                 embeddings.append(output.last_hidden_state.cpu() if torch.cuda.is_available() else output.last_hidden_state)
...
def embed_queries(args, queries, model, tokenizer):
    ...
    with torch.no_grad():
        for k, q in enumerate([queries]):
            ...
            print('Length of batch questions is '),len(batch_question)
            if len(batch_question) == 1 or k == len(queries) - 1:
                print('We entered into the if loop of encoded')
                encoded_batch = tokenizer.batch_encode_plus(
                    batch_question,
                    return_tensors="pt",
                    max_length=args.question_maxlength,
                ...
                # Mean pooling: Average over the sequence length
                pooled_embeddings = output.last_hidden_state.mean(dim=1)

                # Alternatively, use CLS token
                # pooled_embeddings = output.last_hidden_state[:, 0, :]  # CLS token

                embeddings.append(pooled_embeddings.cpu())  # Ensure tensor is on CPU

                batch_question = []
            else:
                print('We do not entered into the if condition of encoded batch')


    embeddings = torch.cat(embeddings, dim=0)
    print(f"Questions embeddings shape: {embeddings.size()}")  # Should now be [n, 768]
